...
int main(int argc, char **argv)
{
    ...
    double viewpos_follow[3] = {-2.09, 0.00201, 0.479 + H[0] * scale + altimetric_compensation};
    ...
    while (wb_robot_step(TIME_STEP) != -1)
    {
        ...
        while (key > 0)
        {
            ...
            case WB_KEYBOARD_LEFT:
                ...
                wb_supervisor_field_set_sf_vec3f(pos, data_pos);
                wb_supervisor_field_set_sf_rotation(rot, data_rot);
                printf("t=%.3f,H = %f,Pitch = %f,Roll = %f,Yaw = %f\n", t[index], H[index], euler.Pitch, euler.Roll, euler.Yaw);
                break;
...
void quaternion_euler(QUATERNION_t *quaternion)
{
    ...
    euler.Roll = (float)(atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2 * q2 + 1) * degrees);
    euler.Yaw = (float)(atan2(2 * (q1 * q2 + q0 * q3), q0 * q0 + q1 * q1 - q2 * q2 - q3 * q3) * degrees);
}
